...
export default function App() {
  ...
  const [selectedUndoLogId, setSelectedUndoLogId] = useState<string | null>(null);
  ...
  function isLogAlreadyUndone(logId: string) {
    return game.log.some((l) => l.text.includes("UNDO ACTION " + logId + ":"));
  }

  function isLogUndoable(logItem: LogItem) {
    if (logItem.kind === "undo" || logItem.kind === "warn" || logItem.kind === "info") return false;
    if (isLogAlreadyUndone(logItem.id)) return false;

    return (
      /^(.+) buy-in confirmed/.test(logItem.text) ||
      /^(.+) cash out confirmed/.test(logItem.text) ||
      /^(.+) host fee marked paid/.test(logItem.text) ||
      /^(.+) host fee marked unpaid/.test(logItem.text) ||
      /^(.+) pending buy-in created/.test(logItem.text)
    );
  }

  function undoSpecificLogAction(logId: string) {
    updateGame((g) => {
      const target = g.log.find((l) => l.id === logId);
      if (!target) {
        addLog(g, "UNDO attempted, but the selected action was not found.", "warn");
        return;
      }

      if (g.log.some((l) => l.text.includes("UNDO ACTION " + logId + ":"))) {
        addLog(g, "UNDO attempted, but this action was already undone.", "warn");
        return;
      }

      const buyMatch = target.text.match(/^(.+) buy-in confirmed/);
      const cashMatch = target.text.match(/^(.+) cash out confirmed/);
      const hostPaidMatch = target.text.match(/^(.+) host fee marked paid/);
      const hostUnpaidMatch = target.text.match(/^(.+) host fee marked unpaid/);
      const pendingMatch = target.text.match(/^(.+) pending buy-in created/);

      if (buyMatch) {
        const player = g.players.find((p) => p.name === buyMatch[1]);
        if (player && player.buyIns > 0) {
          player.buyIns -= 1;
          if (target.text.includes("host fee collected")) {
            player.hostFeePaid = false;
          }
          addLog(g, "UNDO ACTION " + target.id + ": reversed buy-in for " + player.name + ".", "undo");
        } else {
          addLog(g, "UNDO ACTION " + target.id + ": buy-in undo failed because player/buy-in was not found.", "warn");
        }
      } else if (cashMatch) {
        const player = g.players.find((p) => p.name === cashMatch[1]);
        if (player) {
          player.cashOut = "";
          addLog(g, "UNDO ACTION " + target.id + ": removed cash out for " + player.name + ".", "undo");
        } else {
          addLog(g, "UNDO ACTION " + target.id + ": cash-out undo failed because player was not found.", "warn");
        }
      } else if (hostPaidMatch) {
        const player = g.players.find((p) => p.name === hostPaidMatch[1]);
        if (player) {
          player.hostFeePaid = false;
          addLog(g, "UNDO ACTION " + target.id + ": removed host fee paid mark for " + player.name + ".", "undo");
        }
      } else if (hostUnpaidMatch) {
        const player = g.players.find((p) => p.name === hostUnpaidMatch[1]);
        if (player && player.name !== g.host) {
          player.hostFeePaid = true;
          addLog(g, "UNDO ACTION " + target.id + ": restored host fee paid mark for " + player.name + ".", "undo");
        }
        ...
        if (pending) {
          ...
          addLog(g, "UNDO ACTION " + target.id + ": removed pending buy-in for " + playerName + ".", "undo");
        } else {
          addLog(g, "UNDO ACTION " + target.id + ": pending buy-in was already gone.", "undo");
        }
      } else {
        addLog(g, "UNDO ACTION " + target.id + ": selected action was not undoable.", "warn");
      }
    });

    setSelectedUndoLogId(null);
    setShowUndo(false);
  }
  ...
  const selectedUndoLog = selectedUndoLogId ? game.log.find((l) => l.id === selectedUndoLogId) : null;
  ...
  return (
    <div className="app-shell">
      <div className="top-card sticky">
        <div className="top-row compact-top-row">
          <div className="top-title-wrap">
            <div className="title">{game.date}</div>
          ...
          <div className="menu-wrap">
            ...
            {showTopMenu && (
              <div className="top-menu">
                <button onClick={() => { updateGame((g) => { g.settings.voice = !g.settings.voice; }); setShowTopMenu(false); }}>
                  {game.settings.voice ? <Mic size={18} /> : <MicOff size={18} />}
                  {game.settings.voice ? "Mute voice" : "Voice on"}
                </button>

                <button onClick={() => { setScreen(screen === "results" ? "live" : "results"); setShowTopMenu(false); }}>
                  <History size={18} /> Results
                </button>

                <button onClick={() => { requestNewGame(); setShowTopMenu(false); }}>
        ...
        <div className="stats-grid top-stats three">
          <Stat label="Bank" value={money(totals.bank)} />
          <Stat label="Cash Out" value={money(totals.cashOut)} />
          <div className="stat host-stat">
            <span>Host Fees</span>
            <b>{money(totals.hostFeesCollected)}</b>
            <em>{totals.hostPaidCount}/{totals.hostOwedCount}</em>
          </div>
        </div>

        {screen === "live" && (
          <div className="action-grid five-actions sticky-actions">
...
<PokerChart game={game} />
      ...
      <AnimatePresence>
        ...
        {showUndo && (
          <Modal
            title={selectedUndoLog ? "Confirm Undo" : "Select Action to Undo"}
            onClose={() => {
              setSelectedUndoLogId(null);
              setShowUndo(false);
            }}
            ...
            {!selectedUndoLog ? (
              <div className="undo-list">
                <div className="warning-box">
                  Select the exact action that needs to be reversed. This is safer than only undoing the last action.
                </div>

                {game.log.filter((l) => isLogUndoable(l)).length === 0 ? (
                  <div className="muted">No undoable actions available.</div>
                ) : (
                  game.log.filter((l) => isLogUndoable(l)).map((l) => (
                    <button key={l.id} className={"undo-item " + l.kind} onClick={() => setSelectedUndoLogId(l.id)}>
                      <b>{l.time}</b>
                      <span>{l.text}</span>
                    </button>
                  ))
              ...
              <div className="undo-confirm-danger">
                <div className="undo-danger-title">CONFIRM UNDO</div>
                <div className="undo-danger-text">
                  You are about to reverse this action:
                </div>
                <div className="undo-selected-action">
                  <b>{selectedUndoLog.time}</b>
                  <span>{selectedUndoLog.text}</span>
                </div>
                <button className="danger big" onClick={() => undoSpecificLogAction(selectedUndoLog.id)}>
                  YES, UNDO THIS ACTION
                </button>
                <button className="neutral big" onClick={() => setSelectedUndoLogId(null)}>
                  Go Back
                </button>
              </div>
            )}
...
function PokerChart({ game }: { game: Game }) {
  const showLate = Boolean((game as any).lateFeeEnabled);

  return (
    <div className="card chart-card compact-chart-card">
      <table className={showLate ? "compact-table has-late" : "compact-table no-late"}>
        <thead>
          <tr>
            <th className="col-player">Player</th>
            <th className="col-buys">Buys</th>
            <th className="col-bought">Bought</th>
            {showLate && <th className="col-late">Late</th>}
            <th className="col-cash">Cash</th>
            <th className="col-balance">Bal</th>
          </tr>
        </thead>
        <tbody>
          {game.players.map((p) => {
            ...
            const latePaid = Number((p as any).lateFeePaid || 0);

            return (
              <tr key={p.name}>
                <td className="compact-name name-with-mark">
                  <b>{p.name}</b>
                  {p.name === game.host ? (
                    <span className="corner-mark host-dash">-</span>
                  ) : p.hostFeePaid ? (
                    <span className="corner-mark host-check">?</span>
                  ) : null}
                </td>
                ...
                {showLate && (
                  <td className="late-fee-cell">
                    <b>{latePaid ? money(latePaid) : ""}</b>
                  </td>
                )}

                <td><b>{p.cashOut !== "" ? money(p.cashOut) : ""}</b></td>
                <td className={balance === null ? "" : balance > 0 ? "pos" : balance < 0 ? "neg" : ""}>
                  <b>{balance === null ? "" : `${balance > 0 ? "+" : ""}${money(balance)}`}</b>
                </td>
              </tr>
            );
          })}
